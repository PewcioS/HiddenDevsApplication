...
Players.PlayerRemoving:Connect(function(player)
	for LobbyName, LobbyData in pairs(LobbysDataTable) do
		local playerIndex = table.find(LobbyData.Players, player.Name)
		if playerIndex then
...
for _,Lobby in pairs(LobbysFolder:GetChildren()) do
	...
	LobbyEnter.Touched:Connect(function(hit)
		...
		if not db and not (#LobbyData.Players >= LobbyData.Info.MaxPlayers) 
			and Humanoid and Humanoid.Health ~= 0 
			and not table.find(LobbyData.Players,hit.Parent.Name) and LobbyData.Info.CanEnter then
...
function TeleportPlayer(Data : {},LobbyData)
	task.wait(1)
